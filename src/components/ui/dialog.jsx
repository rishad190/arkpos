...
const Dialog = React.forwardRef(({ onOpenChange, ...props }, ref) => (
  <DialogPrimitive.Root onOpenChange={onOpenChange} {...props} ref={ref}>
    <div data-dialog-wrapper>{props.children}</div>
...
const DialogContent = React.forwardRef(
  ({ className, children, ...props }, ref) => {
    ...
    const handleCloseAutoFocus = React.useCallback((event) => {
      // Prevent Radix from moving focus automatically; restore it ourselves
      ...
      // Fallback: try to focus a dialog trigger, else body
      const maybeTrigger = document.querySelector(
        "[data-radix-dialog-trigger]"
      );
      if (maybeTrigger && maybeTrigger instanceof HTMLElement) {
        maybeTrigger.focus();
