...
export const useAppStore = create((set, get) => {
  // 1. Instantiate the service HERE, using this store's set/get
  const atomicOperations = new AtomicOperationService(set, get);
  ...
  return {
    ...
    // 2. Expose the initialized instance to other stores
    atomicOperations: atomicOperations,

    setPartnerProducts: (partnerProducts) =>
      set({ partnerProducts, loading: false }),
    ...
    addPartnerProduct: async (productData) => {
      try {
        // 3. Use the instance from the closure
        ...
        return await atomicOperations.execute(
          ...
          async () => {
            ...
            if (snapshot.exists()) {
              ...
              await update(productRef, {
                partnerAccounts: updatedPartnerAccounts,
              });
            }
          }
        );
        ...
        return await atomicOperations.execute(
          ...
          async () => {
            ...
            if (snapshot.exists()) {
              const productData = snapshot.val();
              const partnerAccounts = productData.partnerAccounts || [];
              const partnerIndex = partnerAccounts.findIndex(
      ...
      try {
        return await atomicOperations.execute("deletePartner", async () => {
          ...
          if (snapshot.exists()) {
            const productData = snapshot.val();
            const partnerAccounts = productData.partnerAccounts || [];
            const updatedPartnerAccounts = partnerAccounts.filter(
              (p) => p.name !== partnerName
            );
            await update(productRef, {
              partnerAccounts: updatedPartnerAccounts,
    ...
    updatePartnerTransaction: async (
      productId,
      partnerName,
      transactionId,
      newTransactionData
        ...
        return await atomicOperations.execute(
          ...
          async () => {
            ...
            if (snapshot.exists()) {
              ...
              const transactions =
                partnerAccounts[partnerIndex].transactions || [];
